...
import { useReservations, useReservationsByDate } from "@/lib/queries/hooks/useReservations";
...
export default function AppointmentManagementPage() {
  ...
  const { data: dateReservations, isLoading: dateReservationsLoading } = useReservationsByDate(
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-sm font-medium">
                    Jadwal Waktu - {format(selectedDate, "EEEE, dd MMMM yyyy", { locale: id })}
                  </label>
                  ...
                  <div className="border rounded-lg max-h-96 overflow-y-auto">
                    <div className="grid grid-cols-1 gap-1 p-2">
                      {generateTimeSlots().map((slot, index) => {
                        const isOccupied = isTimeSlotOccupied(slot.startTime, slot.endTime);
                        const occupiedInfo = getOccupiedSlotInfo(slot.startTime, slot.endTime);
                                ...
                                <div>
                                  ...
                                  {isOccupied && occupiedInfo && (
                                    <div className="text-xs text-red-600">
                                      {occupiedInfo.patientName} - {occupiedInfo.complaint}
                                    </div>
                                  )}
                                  {isSelected && (
                                    <div className="text-xs text-green-600">
                                      Slot terpilih untuk {selectedReservation?.name}
                                    </div>
                                  )}
                                </div>
                              </div>
                    ...
                    <div className="border-t p-3 bg-gray-50">
                      <div className="text-xs font-medium mb-2">Keterangan:</div>
